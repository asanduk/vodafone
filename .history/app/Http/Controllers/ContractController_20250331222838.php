...
class ContractController extends Controller
{
    ...
    {
        try {
            \Log::info('Dashboard method started'); // Debug log

            $categories = Category::withCount('contracts')
                ->whereNull('parent_id')
                ->get();
            \Log::info('Categories loaded', ['count' => $categories->count()]); // Debug log
            ...
            \Log::info('All data loaded successfully'); // Debug log
            ...
            \Log::error('Dashboard error', ['error' => $e->getMessage()]); // Debug log
            return view('dashboard')->with('error', 'Daten konnten nicht geladen werden: ' . $e->getMessage());
        }
    }
