...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Debug: Kullanıcı ID'sini ve tüm sözleşmeleri kontrol et
        \Log::info('User ID:', ['id' => $userId]);
        
        // Tüm sözleşmeleri kontrol et
        $allContracts = DB::table('contracts')
            ->where('user_id', $userId)
            ->select('id', 'user_id', 'contract_date', 'commission_amount')
            ->get();
        
        \Log::info('All Contracts:', ['contracts' => $allContracts->toArray()]);

        // Bugünün sözleşmelerini kontrol et
        $todayContracts = DB::table('contracts')
            ->where('user_id', $userId)
            ->whereDate('contract_date', Carbon::today())
            ->select('id', 'contract_date', 'commission_amount')
            ->get();
        
        \Log::info('Today Contracts:', ['contracts' => $todayContracts->toArray()]);
