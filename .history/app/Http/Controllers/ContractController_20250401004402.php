...
class ContractController extends Controller
{
    ...
    {
        $query = Contract::query()->with(['category', 'subcategory']);
    ...
    public function dashboard()
    {
        try {
            ...
            // Bugünkü komisyon
            $dailyCommission = Contract::whereDate('contract_date', Carbon::today())
            ...
            // Bu ayki komisyon
            $monthlyCommission = Contract::whereMonth('contract_date', Carbon::now()->month)
                ->whereYear('contract_date', Carbon::now()->year)
                ->sum('commission_amount') ?? 0;

            // Bu yıldaki komisyon
            $yearlyCommission = Contract::whereYear('contract_date', Carbon::now()->year)
                ->sum('commission_amount') ?? 0;
