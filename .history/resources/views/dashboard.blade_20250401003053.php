<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Debug Information -->
            @if(Auth::user()->is_admin && isset($allContracts))
                <div class="bg-gray-100 p-4 mt-8 rounded">
                    <h3 class="font-bold mb-2">Debug Information:</h3>
                    <div class="overflow-x-auto">
                        <h4 class="font-semibold mt-2">All Contracts:</h4>
                        <pre class="text-xs">{{ print_r($allContracts->toArray(), true) }}</pre>
                        
                        <h4 class="font-semibold mt-2">Today Stats:</h4>
                        <pre class="text-xs">{{ print_r($todayStats, true) }}</pre>
                        
                        <h4 class="font-semibold mt-2">Week Stats:</h4>
                        <pre class="text-xs">{{ print_r($weekStats, true) }}</pre>
                        
                        <h4 class="font-semibold mt-2">Month Stats:</h4>
                        <pre class="text-xs">{{ print_r($monthStats, true) }}</pre>
                        
                        <h4 class="font-semibold mt-2">Year Stats:</h4>
                        <pre class="text-xs">{{ print_r($yearStats, true) }}</pre>
                    </div>
                </div>
            @endif

            <!-- Debug Table - Admin için -->
            @if(Auth::user()->is_admin)
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 mt-8">
                    <h3 class="text-lg font-semibold mb-4">Tüm Sözleşmeler (Debug)</h3>
                    <div class="overflow-x-auto">
