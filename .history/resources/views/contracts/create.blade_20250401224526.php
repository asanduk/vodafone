<x-app-layout>
    ...
    <script>
        ...
        function filterSubcategories(searchText) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            subcategorySearch.addEventListener('input', async function() {
                const categoryId = categorySelect.value;
                if (!categoryId) return;

                const searchText = this.value;
                if (searchText.length < 2) {
                    // Arama metni çok kısaysa tüm alt kategorileri göster
                ...
                try {
                    const response = await fetch(`/categories/${categoryId}/search-subcategories?search=${encodeURIComponent(searchText)}`);
                    const subcategories = await response.json();
                    updateSubcategoryOptions(subcategories);
                } catch (error) {
            ...
            // Alt kategori seçildiğinde arama alanını temizle ve komisyonu hesapla
