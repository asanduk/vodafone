<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form id="contract-form" action="{{ route('contracts.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="flex justify-between mt-6">
                        <button type="button" id="prev-step" class="bg-gray-500 hover:bg-gray-700 text-white px-6 py-2 rounded hidden">
                            <i class="fas fa-arrow-left mr-2"></i>Zurück
                        </button>
                        <button type="button" id="next-step" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded">
                            Weiter<i class="fas fa-arrow-right ml-2"></i>
                        </button>
                        <button type="submit" id="submit-form" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded hidden">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Form gönderilmeden önce özeti güncelle
            form.addEventListener('submit', function(e) {
                ...
                summary.innerHTML = `
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>${categorySelect.options[categorySelect.selectedIndex].text}</div>
                        <div class="font-semibold">Unterkategorie:</div>
                        <div>${subcategorySelect.options[subcategorySelect.selectedIndex].text}</div>
                        <div class="font-semibold">Vertragsdatum:</div>
                        <div>${document.getElementById('contract_date').value}</div>
                        <div class="font-semibold">Vertragsnummer:</div>
                        <div>${document.getElementById('contract_number').value || '-'}</div>
                        <div class="font-semibold">Kundenname:</div>
                        <div>${document.getElementById('customer_name').value || '-'}</div>
                        <div class="font-semibold">Provision:</div>
                        <div>${document.getElementById('commission_amount').value}€</div>
                    </div>
                `;
                
                if (confirm('Möchten Sie den Vertrag jetzt erstellen?')) {
                    form.submit();
                }
            });
